...
{
	public:
		int x, y, center_x, center_y; // Holds postions of center and current
		Ball_Direction direction; // Holds the direction of motion of the ball
		
		
		// Constructor to initialize the position of the ball
		Ball(int x, int y)
		{
			// center_X and center_y store the initial value
			this->center_x = x; 
			this->center_y = y;
			// x and y store current value of the ball position
			this->x = x; 
			this->y = y;
			// ball is initialized static
			this->direction = STOP; 
		}
		
		
		void reset_ball() // Function to bring the ball back to its starting position (center)
		{
			x = center_x;
			y = center_y;
			// To stop the motion of ball
			direction = STOP; 
		}
		
		
		void change_ball_direction(Ball_Direction d) // Function to change direction of ball when it hits either a wall or a blade
		{
			direction = d;
		}
		
		void randomize_ball_direction() // Function to initialize the direction of ball randomly
		{
			direction = (Ball_Direction)((rand()%6) + 1);
		}
		
		
		void move_ball() // Function to move ball around the screen
		{
		 // x increases along right
		 // y increases along bottom
			switch(direction)
			{ 
				case STOP:
					break;
...
{ // Blade Class
	public:
		int x, y, initial_x, initial_y; // Holds the current and default position of blades
...
class Ping_Pong
{
	public:
		...
		// height and width of the Ping_Pong area
		// scores  of player 1 and player 2
		
		char up_1, down_1, up_2, down_2; 
		// keys used by players to move up or down
		
		bool terminate; 
		// to terminate the Ping_Pong
		...
		void draw_layout()
		{
			...
			{ // to traverse along height
				for(int j=0; j<width; j++)
				{ // to traverse along width
		...
		void play() 		// function to respond to player inputs
		{		
		...
		// function to call the functions
		void lets_ping_pong(){
			while(!terminate){
				draw_layout();
				play();
...
// driver function
int main(){
	Ping_Pong game(40, 20); // instantiate game
